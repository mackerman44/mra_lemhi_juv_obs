...
setwd("C:/Users/lottb/Documents/GitProjects/Lemhi_observed_juv/Data/Abundance")

# read in the fish capture csv and filter it a little
bio <- read.csv("Restoration_Site_Fish.csv",
                na.strings=" ",
                header=TRUE) %>%
...
allFish = bio %>%
  ...
  tbl_df() %>%
  select(Species, Year, PitTagID, SubSite) %>%
  distinct() %>%
  mutate(seen = 1) %>%
  spread(SubSite, seen, fill = 0) %>%
...
abund_df = capHist %>%
  ...
  bind_rows(capHist %>%
              filter(R == 0)) %>%
  arrange(Species, Year, SubSite)


# save as .csv file
write.csv(abund_df,
          'Rest_site_abundances.csv',
          row.names = F)
write.csv(movementsummary,
          'Rest_site_Movement.csv')
